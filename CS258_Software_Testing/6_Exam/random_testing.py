...
def random_test():
	...
	for action in range(num_actions):
		if (random.random() < 0.5):
			random_input = int(1000 * random.random()) + 1
			try:
				q.enqueue(random_input)
				list_of_actions.append((random_input, 0))
			except AssertionError, e:
				list_of_actions.append((random_input, 1))

		else:
			try:
				q.dequeue()
				list_of_actions.append(('dq', 0))
			except AssertionError, e:
				list_of_actions.append(('dq', 1))
