...
def test():
	...
	# Root node, nothing else, find before/after
	st.insert(20)
	assert st.find(10) == None
	assert st.find(30) == None
	st.remove(20)
	assert st.isEmpty() == True

	# Two nodes (unbalanced), find before/after
	st.insert(10)
	st.insert(20)
	assert st.find(5) == None
	assert st.find(25) == None
	st.remove(20)
	st.remove(10)
	assert st.isEmpty() == True
	
	# Balanced tree, find before/after
	st.insert(20)
	st.insert(10)
	st.insert(30)
	assert st.find(5) == None
	assert st.find(35) == None
	st.remove(20)
	st.remove(10)
	st.remove(30)
	assert st.isEmpty() == True

	# Left-heavy lopsided insertion
	st.insert(30)
	st.insert(20)
	st.insert(10)
	assert st.findMax() == 30
	assert st.findMin() == 10
	st.remove(30)
	st.remove(20)
	st.remove(10)
	assert st.isEmpty() == True
